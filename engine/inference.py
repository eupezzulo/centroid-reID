...
def inference(
    ...
    if cfg.TEST.WITH_CENTROIDS == 'yes':
        print("Create evaluator for centroids")
        # creates a supervised evaluator that uses centroid-based metric
        evaluator = create_supervised_evaluator(model, metrics={'r1_mAP': R1_mAP_centroids(num_query, max_rank=50, feat_norm=cfg.TEST.FEAT_NORM)},
                                                device=device)
    elif cfg.TEST.RE_RANKING == 'no':
